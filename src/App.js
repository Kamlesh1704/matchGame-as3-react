...
class Matchgame extends Component {
  ...
  clickingthumb = imgUrl => {
    const {image} = this.state
    const randomImg = Math.floor(Math.random() * imagesList.length) + 1
    if (image === imgUrl) {
      this.setState(prevState => ({score: prevState.score + 1}))
      this.setState({image: imagesList[randomImg].imageUrl})
    } else {
  ...
  render() {
    const {isGameInProgress, score, time, activeTabId, image} = this.state
    const filteredImages = this.filtered()
    return (
      <div className="div">
        <nav className="nav">
          <ul className="nav">
        ...
        {isGameInProgress ? (
          <div className="div2">
            <img src={image} className="bigimg" alt="match" />
            <ul className="tabul">
            ...
            <ul className="thumbdiv">
              {filteredImages.map(projectDetails => (
                <ProjectItem
                  key={projectDetails.id}
                  projectDetails={projectDetails}
                  clickingthumb={this.clickingthumb}
                />
              ))}
            </ul>
          </div>
          ...
          <div className="scorecards">
            <div className="card">
              ...
              <p className="score">{score}</p>
              <button className="playagain" onClick={this.reset} type="button">
                <img
                  src="https://assets.ccbp.in/frontend/react-js/match-game-play-again-img.png"
                  alt="reset"
                  className="reset"
                />
                <p className="resetpara">PLAY AGAIN</p>
              </button>
            </div>
          </div>
        )}
